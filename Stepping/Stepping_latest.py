# -*- coding: utf-8 -*-
"""
Created on Sun Feb 25 19:40:29 2024
...
def predict_fault():
    ...
    for i, entry in enumerate(entries):
        ...
        try:
            ...
            input_values[labels[i]] = float_value
        except ValueError:
    ...
    result_label.config(text=f'Predicted Stepping: {prediction[0]:.2f}')
...
logo_label_right.grid(column=2, row=0, padx=10, pady=5, sticky="e")
...
# Create labels and entry fields for input variables in two columns
labels = ['ST', 'BT', 'AD', 'JS', 'AP', 'AT', 'FI', 'CS', 'EM', 'DE', 'MR', 'TS', 'CT', 'AG', 'PN', 'PL']
entries = []

# Create entry tables next to the labels
...
predict_button.grid(column=0, columnspan=4, row=len(labels)+1, pady=10)

# Create a label to display the prediction
result_label = ttk.Label(app, text='', font=("Arial", 14, "bold"))
result_label.grid(column=0, columnspan=4, row=len(labels)+2)
